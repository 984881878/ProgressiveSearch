...
class DataProvider:
    ...
    def random_sample_valid_set(train_labels, valid_size, n_classes):
        ...
        per_class_remain = get_split_list(valid_size, n_classes)  # 评估集中的图片种类应该每类都是均匀的，即一样多
        ...
        for idx in rand_indexes:
            ...
            if isinstance(label, float):
                ...
                assert isinstance(label, int)  # 将样本标签都转为整数
            if per_class_remain[label] > 0:    # 从训练集中挑出部分样本作为评估及，评估集中每类样本数量相当
